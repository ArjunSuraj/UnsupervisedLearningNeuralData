...
#modified
HMM = True                                   # for with or without spatial correlations,
...
#modified
shuffle = False                                   #  if HMM (t-dep), then don't shuffle for Prentice et al data,
...
maxModes = 150
nModesList = range(1,maxModes+1,10)  # steps of 5, no need to go one by one
...
if len(sys.argv) > 1:
    ...
    interactionFactorIdx = 19       # experimental data
    interactionFactor = interactionFactorList[interactionFactorIdx]
    nSeed = 0
...
def spikeRasterToSpikeTimes(spikeRaster,binsize=binsize):
    ...
    #print('spikeRaster.shape',spikeRaster.shape)
...
def loadDataSet(dataFileBase,interactionFactorIdx,shuffle=True,seed=100):
    ...
    if interactionFactorIdx < 20:
        ...
        spikeRaster_ = database['sample']        # neurons x timebins
        spikeRaster = spikeRaster_[:,0:100000] # was 100000
    ...
    if fitMixMod:
        ...
        if HMM:
            ...
            if crossvalfold > 1:
                ...
                params,trans,P,emiss_prob,alpha,pred_prob,hist,samples,stationary_prob,train_logli_this,test_logli_this = \
                    EMBasins.pyHMM(nrnspiketimes, np.ndarray([0]), np.ndarray([0]),
                                        float(binsize), nModes, niter)
