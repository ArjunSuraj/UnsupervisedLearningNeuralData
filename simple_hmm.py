...
if __name__ == '__main__':
  ...
  n_samples = 2000
  spikeRaster_transpose = np.zeros([n_samples,2]) 
  spikeRaster_transpose[0,:] = [1,0]
  #np.array([1,0])
  for i in range(1,n_samples):
    if(np.random.uniform() < 0.25):
      spikeRaster_transpose[i,:] = spikeRaster_transpose[i-1,:]
    else:
      spikeRaster_transpose[i,:] = 1 - spikeRaster_transpose[i-1,:]
  ...
  print('after dividing', train_score/n_samples)
